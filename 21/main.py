...
def main():
    ...
    ip = 0
    reg4s = set()
    last_added_reg4 = None

    while True:
        try:
            if ip == 28:
                if register[4] in reg4s:
                    print(register)
                    print('reg4 already registered, last added was {}'.format(last_added_reg4))
                    break
                else:
                    reg4s.add(register[4])
                    last_added_reg4 = register[4]
                    # print(register, len(reg4s))
